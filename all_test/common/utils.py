...
def get_provider(device):
    provider_dict = {
        'dtu': 'TopsInferenceExecutionProvider',
...
def get_dtu_clock_freq():
    try:
        proc = subprocess.Popen(
            ["efsmt -clock list |grep DTU |awk -F' ' '{print $5}'"],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        dtu_clock = proc.communicate()[0].strip()
        dtu_clock = dtu_clock.decode('utf-8')
    except Exception as ex:
        # traceback.print_exc()
        dtu_clock = None
    return dtu_clock
...
def get_environment_info():
    ...
    freq = get_dtu_clock_freq()
    if freq:
        base_dict.update({'DTU_clock': '{}MHz'.format(freq)})
    hbm = get_hbm_phy()
    if hbm:
