...
class CascadeRcnnFactory(OnnxModelFactory):
    model = "cascade_rcnn"
...
class CascadeRcnn(OnnxModel):
    ...
    def __imresize(self, item):
        ...
        img = item.data
        h, w = img.shape[:2]
        new_w, new_h = self.options.get_scale()
    ...
    def create_dataset(self):
        ...
        return read_dataset(self.anno.getImgIds()[:10])
    ...
    def load_data(self, img_id):
        ...
        return CascadeRcnnItem(img, metas, img_id)
    ...
    def run_internal(self, sess, items):
        ...
        items[0].outputs = sess.run(output_names, input_feed)
        return items
    ...
    def postprocess(self, item):
        ...
        for i in range(batch_size):
            batch_dets, batch_labels = item.outputs[:2]
            dets = batch_dets[i].copy()
            labels = batch_labels[i].copy()

            batch_masks = None if len(item.outputs) == 2 else item.outputs[2]
            ...
            if batch_masks is not None:
                ...
                mask_results = []
                for cls_id in range(num_classes):
                    masks = batch_masks[i][labels == cls_id, :]
                    if masks.size == 0:
                        ...
                        for mask in masks:
                            if post_rescale:
                                ...
                                cls_mask = cls_mask > mask_binary_thr
        ...
        for det in result_a:
            ...
            for label, bboxes in enumerate(all_bboxes):
                ...
                for i in range(0, bboxes.shape[0]):
                    ...
                    # f.write(str([xmin, ymin, xmax - xmin, ymax - ymin]) + '  ' + str(score) + '  ' + str(label) + '\n')
                    # f.close
